...
const Layout: React.FC<LayoutProps> = ({ children, currentView, setView, onOpenSettings }) => {
  const navItems = [
    ...
    <div className="min-h-screen bg-[conic-gradient(at_top_right,_var(--tw-gradient-stops))] from-indigo-50 via-purple-50 to-amber-50 text-slate-800 font-sans selection:bg-purple-200">
          ...
          <div className="flex justify-between h-20 items-center">
            ...
            <div className="flex items-center space-x-2 pl-4 border-l border-indigo-100/50">
               <button 
                  ...
                  className="flex items-center space-x-2 px-4 py-2 rounded-full bg-white/50 border border-white hover:bg-white hover:shadow-md transition-all text-sm font-bold text-slate-600 hover:text-indigo-600 group"
      ...
      <footer className="hidden md:block text-center py-10 text-slate-400 text-sm relative z-10">
        <div className="flex items-center justify-center space-x-3 mb-3">
           ...
           <span className="font-fantasy text-indigo-300 text-lg">Magic in every pixel</span>
           <span className="h-px w-12 bg-indigo-200"></span>
        </div>
        <p className="opacity-60 font-medium">Kid-Safe Guarantee • Private & Secure • Powered by Google Gemini</p>
      </footer>
    </div>
  );
};
